...
def cuda_device_deps(family, version):
    ...
    native.cc_library(
        name = "cuda",
        hdrs = cuda_hdrs,
        srcs = [cuda_driver_so_path(version)],
    ...
    native.genrule(
        name = "cudnn_copy_includes",
        srcs = [cudnn_include_prefix + "/cudnn_v7.h"],
        outs = [cudnn_include_prefix + "/cudnn.h"],
        cmd = "cp $< $@",
    )
...
def _cuda_select(target):
    return select({
        ...
        "//engine/build:platform_jetpack42": ["@cuda_aarch64_jetpack42//:" + target],
    })
