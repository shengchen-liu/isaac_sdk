...
LSM6DS33::LSM6DS33() {
  memset(&LSM6DS33_state_, 0, sizeof(LSM6DS33_state_));
...
void LSM6DS33::configureResponses() {
...
bool LSM6DS33::sendCommandU(uint16_t command_id, uint32_t value1, uint32_t value2) {
...
bool LSM6DS33::sendCommandF(uint16_t command_id, float value1, float value2) {
  rmp_set_packet_t packet;
  serialization::uint16_t_to_bytes(command_id, packet.messageId);
...
bool LSM6DS33::sendSpeedCommand(float linear_speed, float angular_speed) {
  // TODO Make sure that there are not magic scale factors necessary.
  return sendCommandF(0x0500, linear_speed, angular_speed);
}

bool LSM6DS33::sendCommandPacketToRmp(const rmp_set_packet_t& packet) {
