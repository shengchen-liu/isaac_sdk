...
import packages.freespace_dnn.apps.training
import json
import numpy as np
import os
import packages.ml
...
flags.DEFINE_string('training_config_path',
                    'packages/freespace_dnn/apps/training/segmentation_training_config.json',
...
def train_and_evaluate():
    ...
    node = app.find_node_by_name("freespace_training.path_segmentation_training_samples")
    ...
    try:
        ...
        train_spec = tf.estimator.TrainSpec(
            input_fn=lambda: get_dataset(bridge), max_steps=num_iterations, hooks=[time_hist])
        eval_spec = tf.estimator.EvalSpec(
            input_fn=lambda: get_dataset(bridge),
