...
function GetStatisticsView() {
  ...
  if (stats_view === null || stats_view === undefined) {
    ...
    // Given a string `text` and a string to match `match`, returns a new string that enlights the
    // original text where it matches the match. It attempts to match using the fewer number of
    // splits of `match`.
    // If there is not valid match, it returns an empty string.
    stats_view.filter = function(text, match) {
      ...
      // Needs to be bigger than the biggest valid match possible.
      const infinity = 10 + match_size;
      ...
      for (let t = 0; t < text_size;) {
        if (dp[t + 1][m] === dp[t][m]) {
          ...
          ans += "<b><font color='red'>" + text.substring(t, t+i) + "</font></b>";
          t += i;
          m += i;
        }
      }
    ...
    stats_view.sort = function() {
      const sort = function(a, b) {
        ...
        if (stats_view.sort_col < 2) {
          ...
          return parseFloat(b.children[stats_view.sort_col].innerHTML) -
                 parseFloat(a.children[stats_view.sort_col].innerHTML);
        }
      };
      let change = false;
    ...
    stats_view.showCodelet = function(evt) {
      ...
      for (let i = 0; i < tabcontent.length; i++) {
        ...
        let match = stats_view.filter(full_str, stats_view.filter_str);
        bmatch = Math.min(bmatch, match.split);
      }
      for (let i = 0; i < tabcontent.length; i++) {
        const full_str = tabcontent[i].children[0].original_name + "\n" +
                         tabcontent[i].children[1].original_name;
    ...
    WindowManager().createWindow(stats_view, "Statistics");
    // Remove the legend
...
function ResetStatisticsView() {
  ...
  let view = GetStatisticsView();
  for (let node in view.node_codelets) {
    for (let codelet in view.node_codelets[node]) {
      let div = view.node_codelets[node][codelet];
