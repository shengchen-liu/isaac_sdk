...
class Renderer2D extends SightRenderer {
  ...
  applyInput() {
    ...
    let world_T_baseReferentialFrame =
        PoseTree().get("world", this.base_referential_frame_, this.current_time_);
    if (!world_T_baseReferentialFrame || !this.baseReferentialFrame_T_current_) return;
    ...
    if (this.cursorPositionCanvas_) {
      ...
      // Cursor ray start point in "current coordinate system plus z axis" space.
      const cursorRayStartCurrent3D =
            new THREE.Vector3(
              cursorPointCurrent.x,
              cursorPointCurrent.y,
              0
            );
      // Cursor ray end point in "current coordinate system plus z axis" space.
      const cursorRayEndCurrent3D =
            new THREE.Vector3(
              cursorPointCurrent.x,
              cursorPointCurrent.y,
                -1
      ...
      let cursorRayStartBaseReferentialFrameHomogeneous =
          new THREE.Vector4(
            cursorRayStartCurrent3D.x,
            cursorRayStartCurrent3D.y,
            cursorRayStartCurrent3D.z,
            1.0
          ).applyMatrix4(this.baseReferentialFrame_T_current_);
      // Cursor ray end in homogeneous base referential frame coordinates.
      let cursorRayEndBaseReferentialFrameHomogeneous =
          new THREE.Vector4(
            cursorRayEndCurrent3D.x,
            cursorRayEndCurrent3D.y,
            cursorRayEndCurrent3D.z,
            1.0).applyMatrix4(this.baseReferentialFrame_T_current_);
    ...
    if (this.cursorPositionCanvas_) {
      for (let i in this.markers_channels_) {
        ...
        let world_T_baseReferentialFrame_translation = new THREE.Vector3();
        let world_T_baseReferentialFrame_orientation = new THREE.Quaternion();
        let world_T_baseReferentialFrame_scale = new THREE.Vector3();
        world_T_baseReferentialFrame.decompose(
            world_T_baseReferentialFrame_translation,
            world_T_baseReferentialFrame_orientation,
            world_T_baseReferentialFrame_scale
        );
        let geometry = this.markerGeometryCanvas_(transformBaseReferentialFrame,
            world_T_baseReferentialFrame_orientation, scaleFactor);
        let positionBaseReferentialFrame = transformBaseReferentialFrame.position;
        let positionCanvas = this.baseReferentialFrameToCanvas_(positionBaseReferentialFrame);
  ...
  render(op, time) {
    ...
    if (op.p) {
      ...
      if (op.p.t === "c") {
        ...
        this.current_transformation_ =
            SightChannel.GetTransform(op, this.base_referential_frame_, this.static_frame_,
        ...
        this.projection_.multiply(SightChannel.GetTransform(
              op, this.base_referential_frame_, this.static_frame_, this.current_time_, time));
        this.current_transformation_.multiply(
            SightChannel.GetTransform(op, this.base_referential_frame_, this.static_frame_,
                                      this.current_time_, time));
      }
    }
  ...
  resetCameraProjection() {
    let baseTransformation =
        this.channels_[0].c.getTransform(this.base_referential_frame_, this.static_frame_,
                                         this.current_time_);
    ...
    this.baseReferentialFrame_T_current_ =
        baseTransformation.clone().multiply(
  ...
  draw_() {
    ...
    let world_T_baseReferentialFrame =
        PoseTree().get("world", this.base_referential_frame_, this.current_time_);
    if (world_T_baseReferentialFrame) {
      ...
      this.drawMarkersImage_(markers_ctx, world_T_baseReferentialFrame_orientation);
  ...
  drawImage_(op) {
    ...
    if (true) { // CHECK identity
      const s = this.current_scale;
      const pose = Renderer2D.GetPose2(this.current_transformation_);
      this.ctx_.save();
  ...
  drawCircle_(op) {
    ...
    this.ctx_.closePath();
    this.ctx_.lineWidth = this.current_size_;
  ...
  drawRect_(op) {
    ...
    // FIXME: display a cuboid if needed
    const pt1 = this.transformPoint_(x1, y1, 0);
    const pt2 = this.transformPoint_(x1, y2, 0);
    const pt3 = this.transformPoint_(x2, y2, 0);
    const pt4 = this.transformPoint_(x2, y1, 0);
    this.ctx_.lineWidth = this.current_size_;
    this.ctx_.beginPath();
  ...
  // Draw Asset
  drawAsset_(op) {
    const asset = AssetManager().getAsset2D(op.n);
    if (asset && asset.img) {
      this.current_scale *= asset.img.scale;
      this.drawImage_(asset);
      this.current_scale /= asset.img.scale;
    } else {
  ...
  markerGeometryCanvas_(transform, orientation, scaleFactor) {
    // Convert a point in the marker's space to canvas space
    let markerToCanvas = (pMarker) => {
      // This quaternion is the world orientation of our marker.
      const orientationWorld = orientation.clone();
      orientationWorld.multiply(transform.orientation.clone());
      // Align with the world z axis, by applying the shortest possible rotation.
      // The shortest rotation from u to v is normalized(1 + <u, v> + u x v), so our 2D rotation
      // is rotation by normalized(1 + <qkq^*, k> + qkq^* x k) q = ... = normalized(qw + kqz).
      const alignedWorld = new THREE.Quaternion(0, 0, orientationWorld.z, orientationWorld.w);
      alignedWorld.normalize();
      // Transform back to the base referential frame
      const alignedOrientationBRF = orientation.clone().conjugate();
      alignedOrientationBRF.multiply(alignedWorld);
      const baseReferentialFrame_T_marker =
            new THREE.Matrix4().makeRotationFromQuaternion(alignedOrientationBRF)
            .setPosition(transform.position);
      const pBaseReferentialFrame = pMarker.clone().applyMatrix4(baseReferentialFrame_T_marker);
      const pCanvas = this.baseReferentialFrameToCanvas_(pBaseReferentialFrame);
      return pCanvas;
    };
  ...
  drawMarkersImage_(ctx, markerOrientationBaseReferentialFrame) {
    let drawMarker = (transform, orientation, circleColor, axisColor, scaleFactor) => {
      const geometry = this.markerGeometryCanvas_(transform, orientation, scaleFactor);
    ...
    let drawLabel = (text, transform, orientation, color, scaleFactor) => {
      const geometry = this.markerGeometryCanvas_(transform, orientation, scaleFactor);
    ...
    for (let i in this.markers_channels_) {
      ...
      drawMarker(transform, markerOrientationBaseReferentialFrame, circleColor, axisColor,
          scaleFactor);
      if (name == this.hoveredMarker_) {
        drawLabel(name, transform, markerOrientationBaseReferentialFrame, '#ff0000', scaleFactor);
      }
    }

    let orientationGuideLineSegmentBaseReferentialFrame =
        this.markersWidget_.orientationGuideLineSegment();
    if (orientationGuideLineSegmentBaseReferentialFrame) {
      let start = this.baseReferentialFrameToCanvas_(
          orientationGuideLineSegmentBaseReferentialFrame.start);
      let end = this.baseReferentialFrameToCanvas_(
          orientationGuideLineSegmentBaseReferentialFrame.end);
