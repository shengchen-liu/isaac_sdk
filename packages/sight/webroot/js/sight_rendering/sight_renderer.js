...
class SightRenderer {
  ...
  update(channel) {
    if (this.window_fit_content_ && this.setting_div_.parentNode === null) {
      ...
      const width = Math.max(this.canvas_.width, this.legend_div.clientWidth);
  ...
  updateImpl(channel) {
    ...
    // TODO: allow to set a custom from the config/settings instead of using first channel
    const previous_base_referential_frame = this.base_referential_frame_;
    this.base_referential_frame_ = this.channels_[0].c.getFrame();
    if(previous_base_referential_frame != this.base_referential_frame_) {
