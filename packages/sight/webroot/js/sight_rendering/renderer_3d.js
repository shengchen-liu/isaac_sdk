...
class Renderer3D extends SightRenderer {
    ...
    if (!op.cached_obj_[this.getName()]) {
      let renderImpl = function(op) {
        ...
        if (op.p) {
          ...
          if (op.p.t !== "f") {
            transformation =
                SightChannel.GetTransform(op, that.base_referential_frame_, that.static_frame_,
                                          time, that.current_time_);
          } else {
            frame = op.p.f;
            has_frame = true;
          }
  ...
  setCanvasDimensions(dims) {  // override
    ...
    if (this.camera_ == null) {
      ...
      this.camera_.position.set(40.0, 20.0, 20.0);
  ...
  drawImage_(op) {
    return CreateMap(op.img, 1.0, 20.0);
  }
