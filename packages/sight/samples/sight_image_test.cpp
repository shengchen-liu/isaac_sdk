 ...
 public:
  ...
  Image effect(const Image& img, int start) {
    ...
    for (size_t row = 0; row < img.rows(); row++) {
      const ssize_t dec = 5.0 * std::cos((start + row) * 0.2);
...
class PinholeImageNode : public alice::Codelet {
 public:
    ...
    show("path", [&](sight::Sop& sop) {
      ...
      for (double d = -20.0; d <= 20.0; d += 0.1) {
        const Vector3d pt(d, std::cos(angle_start - d) * 0.5,
                          0.5 * (1.5 + std::sin(angle_start * 0.5)));
  ...
  // Focal of the camera
  ISAAC_PARAM(Vector2d, focal, Vector2d(480.0, 480.0));
  // Image to be laoded
 ...
 public:
  ...
  void effect(int start, sight::Sop& sop) {
    SampleCloud3f points(depth_.rows() * depth_.cols()),
        colors(depth_.rows() * depth_.cols());
    ...
    for (size_t row = 0; row < depth_.rows(); row++) {
      const ssize_t dec = 5.0 * std::cos((start + row) * 0.2);
      for (size_t col = 0; col < depth_.cols(); col++) {
        int rcol = std::max<int>(0, std::min<int>(depth_.cols() - 1, dec + col));
        Vector3f pt = 0.001 * (depth_(row, rcol) * (Vector3f(row, col, 100.0) - center));
...
void UnrealImageToSight(alice::Application& app) {
  ...
  left->set_name("left");
  left->set_image_filename("engine/gems/image/data/left.png");
  left->set_tick_period("50hz");
  auto right = app.createMessageNode("right")->addComponent<ImageNode<Image3ub>>();
  right->set_name("right");
  right->set_image_filename("engine/gems/image/data/right.png");
  right->set_tick_period("50hz");
  auto depth = app.createMessageNode("depth")->addComponent<ImageNode<Image1ub>>();
  depth->set_name("depth");
  depth->set_image_filename("engine/gems/image/data/depth.png");
  depth->set_tick_period("50hz");
  auto label = app.createMessageNode("label")->addComponent<ImageNode<Image3ub>>();
  label->set_name("label");
  label->set_image_filename("engine/gems/image/data/label.png");
  label->set_tick_period("50hz");

  auto cloud3d = app.createMessageNode("cloud3d")->addComponent<View3DClouds>();
  cloud3d->set_name("cloud3d");
  cloud3d->set_path("engine/gems/image/data/");
  cloud3d->set_tick_period("20hz");

  auto pinhole = app.createMessageNode("pinhole")->addComponent<PinholeImageNode>();
  pinhole->set_name("pinhole");
  pinhole->set_path("engine/gems/image/data/stairs.png");
  pinhole->set_tick_period("50hz");
}
