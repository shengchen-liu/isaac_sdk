...
MessageBasePtr Clone(ConstMessageBasePtr message) {
  ...
  for (size_t i = 0; i < result->buffers.size(); i++) {
    ...
    CpuAlignedBuffer lhs(rhs.rows(), rhs.stride());
...
: child_(std::move(child)) {
  proto_id_ = child_->proto_id();
  ::capnp::ReaderOptions options;
  options.traversalLimitInWords = kj::maxValue;
  capnp_reader_ = std::make_unique<::capnp::SegmentArrayMessageReader>(child_->segments(), options);
}

MallocProtoMessage::MallocProtoMessage()
: MallocProtoMessage(std::make_unique<::capnp::MallocMessageBuilder>()) { }

MallocProtoMessage::MallocProtoMessage(
    std::unique_ptr<::capnp::MallocMessageBuilder> malloc_message_builder)
: malloc_message_builder_(std::move(malloc_message_builder)) {
  ASSERT(malloc_message_builder_, "Must not be nullptr");
  ::capnp::ReaderOptions options;
  options.traversalLimitInWords = kj::maxValue;
  capnp_reader_ = std::make_unique<::capnp::SegmentArrayMessageReader>(
      malloc_message_builder_->getSegmentsForOutput(), options);
}

kj::ArrayPtr<const kj::ArrayPtr<const ::capnp::word>>
MallocProtoMessage::segments() const {
  return malloc_message_builder_->getSegmentsForOutput();
}

BufferedProtoMessage::BufferedProtoMessage(std::vector<uint8_t> buffer,
    size_t offset, std::vector<size_t> segment_lengths) {
  ...
  ::capnp::ReaderOptions options;
  options.traversalLimitInWords = kj::maxValue;
  capnp_reader_.reset(new ::capnp::SegmentArrayMessageReader(segments(), options));
}

kj::ArrayPtr<const kj::ArrayPtr<const ::capnp::word>>
BufferedProtoMessage::segments() const {
  return kj::ArrayPtr<const kj::ArrayPtr<const ::capnp::word>>(
      segments_.data(), segments_.size());
}
