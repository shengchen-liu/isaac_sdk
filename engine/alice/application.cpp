...
// From a name 'nodename/compname/tag', return the corresponding component pointer and tag string
std::tuple<Component*, std::string> GetComponentAndTag(Application& app, const std::string& name) {
  auto tokens = Split(name);
  auto* node = app.getNodeByName(std::get<0>(tokens));
...
Application::~Application() {
  ...
  if (!config_backup_.empty()) {
    ApplicationJsonLoader::WriteConfigToFile(config_backup_, backend_->config_backend()->root());
  }
  backend_ = nullptr;
}
...
void Application::createApplication(const ApplicationJsonLoader& loader) {
  ...
  name_ = loader.name_;
  uuid_ = Uuid::Generate();
  backend_ = std::make_unique<Backend>(this, loader.scheduler_json_);
...
void Application::createMore(const ApplicationJsonLoader& loader) {
  ...
  for (const auto& edge : loader.edges_) {
    const auto source = GetComponentAndTag(*this, edge.source);
    const auto target = GetComponentAndTag(*this, edge.target);
