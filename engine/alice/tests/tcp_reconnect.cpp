...
void TcpPublisherDeathImpl(int num_iterations, double duration, int port) {
  ...
  auto pub = [=](int round) {
    ...
    ping_tx->set_round(round);
    ping_tx->set_tick_period(std::to_string(tick_period) + "s");
    Node* remote_node = app.createMessageNode("remote");
    auto* publisher = remote_node->addComponent<TcpPublisher>();
    publisher->set_port(port);
  ...
  auto sub = [=] {
    ...
    subscriber->set_host("localhost");
    subscriber->set_port(port);
    subscriber->set_reconnect_interval(0.1);
