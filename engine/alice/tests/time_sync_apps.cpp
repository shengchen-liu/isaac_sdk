...
void Test1Receiver1Publisher(bool time_sync) {
  ...
  std::thread t2([time_sync] {
    ...
    tcp_sub_comp->set_port(kPort1);
    tcp_sub_comp->set_host("127.0.0.1");
    tcp_sub_comp->set_reconnect_interval(0.1);
    if (time_sync) {
      // Add time-sync to tcp node
...
void Test1Receiver2Publisher(bool time_sync) {
  ...
  std::thread t2([time_sync] {
    ...
    tcp_sub_comp1->set_port(kPort1);
    tcp_sub_comp2->set_port(kPort2);
    tcp_sub_comp1->set_host("127.0.0.1");
    tcp_sub_comp2->set_host("127.0.0.1");
    tcp_sub_comp1->set_reconnect_interval(0.1);
    tcp_sub_comp2->set_reconnect_interval(0.1);
    if (time_sync) {
      // Add time-sync to tcp node
      tcp_sub_node->addComponent<TimeSynchronizer>();
    }
