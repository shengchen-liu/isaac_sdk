...
// Gets the name of a module from a bazel target:
...
std::string GetModuleFilename(const std::string& raw_module) {
  std::string module = raw_module;
  std::string repo_path = "";
  if (StartsWith(module, "@")) {
    size_t repo_end = raw_module.find("/");
    if (repo_end == 0 || repo_end == 1) {
      PANIC("Invalid module name '%s'", raw_module.c_str());
    }
    if (repo_end == std::string::npos) {
      const std::string repo_name = raw_module.substr(1);
      repo_path = std::string(kExternalPath) + repo_name + "/";
      module = "//" + repo_name;
    } else {
      repo_path = std::string(kExternalPath) + raw_module.substr(1, repo_end - 1) + "/";
      module = raw_module.substr(repo_end);
    }
  }

  auto tokens = SplitString(module, ':');
  // First get path and name sections
  ...
  return repo_path + path + "/lib" + name + "_module.so";
}
...
std::optional<std::string> ModuleManager::loadModule(const std::string& name) {
  ...
  const std::string filename = GetModuleFilename(name);
  module->open(module_paths_, filename, working_path_);
  if (!module->isOpen()) {
    if (!filename.empty() && filename.at(0) != '/' &&
        filename.substr(kExternalPathLength).compare(kExternalPath) != 0) {
      // For non-absolute path, try to load from external isaac workspace
      module->open(module_paths_, std::string(kExternalIsaacPath) + filename, working_path_);
    }
    if (!module->isOpen()) {
  ...
  LOG_INFO("Loaded module '%s': Now has %zd components total", filename.c_str(),
