...
struct StreamMessageWriter::Impl {
  std::vector<ByteBufferConstView> proto_blobs;
...
void StreamMessageWriter::write(ConstMessageBasePtr message, TimeSynchronizer* time_synchronizer,
  ...
  for (const auto& buffer : message->buffers) {
    impl_->buffer_blobs.push_back(ByteBufferConstView{
...
class StreamMessageReader::Impl {
 ...
 private:
  ...
  // Buffers to read
  std::vector<ByteBufferView> blobs_;
  std::vector<CpuAlignedBuffer> data_;
  size_t blob_index_;
