...
void NodeBackend::destroyNode(const std::string& name) {
  ...
  Node* node = findNodeByNameImpl(name);
  ...
  // The lambda we create below will take of the remainin steps to destroy the node.
  auto it = std::find_if(nodes_.begin(), nodes_.end(),
      [node](auto& other) { return other.get() == node; });
  // We need to do a bit of an unusual operation here. We want to remove the unique pointer holding
  // the node from the node list. Ideally we would like to give this pointer to our lambda below.
  // However C++ standard does not allow to move a non-copyable lambda into an std::function.
  // Thus we release the unique pointer here and manually delete the node in the lambda below.
  it->release();
  nodes_.erase(it);
...
void NodeBackend::createNodeFromJson(const nlohmann::json& node_json, const std::string& prefix) {
  ...
  const std::string node_name = prefix + *maybe_name;
  // create a new node
...
nlohmann::json NodeBackend::saveToJson() const {
  std::lock_guard<std::mutex> lock(nodes_mutex_);
  nlohmann::json result;
  for (const auto& ptr : nodes_) {
    nlohmann::json node_json = ApplicationJsonLoader::GraphToJson(ptr->getComponents<Component>());
    node_json["name"] = ptr->name();
    result.emplace_back(std::move(node_json));
  }
  return result;
}
...
void NodeBackend::start() {
  ...
  for (auto& node_uptr : nodes_) {
    ...
    start_stop_queue_.push_back([this, node] { startImmediate(node); });
  }
}
...
void NodeBackend::stop() {
  stop_requested_ = true;
  {
    ...
    for (auto it = nodes_.rbegin(); it != nodes_.rend(); ++it) {
      ...
      start_stop_queue_.push_back([this, node] { stopImmediate(node); });
    }
  }
...
void NodeBackend::startComponent(Component* component) {
  ...
  start_stop_queue_.push_back([this, component] { startImmediate(component); });
}
...
void NodeBackend::createComponentImpl(Node* node, Component* component) {
  ...
  // Get the config component. We need to be careful as this does not work if we are only just
  // adding the config component.
  Config* config = dynamic_cast<Config*>(component);
  if (!config) {
    config = &node->config();
  }
  config->set(component, "__type_name", component->type_name());
}
...
void NodeBackend::queueMain() {
  while (queue_running_) {
    processQueue();
    // don't busy loop // FIXME use a condition variable or such
    Sleep(10'000'000);
  }
