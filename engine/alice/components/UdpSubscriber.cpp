...
void UdpSubscriber::start() {
  ...
  impl_->package_reassembler.on_message =
      [this, message_ledger](ProtoMessageBasePtr message, const std::string& channel) {
  ...
  impl_->socket = std::make_unique<asio::ip::udp::socket>(
      backend_->io_service(), asio::ip::udp::endpoint(asio::ip::udp::v4(), port));
  async_receive();
}
