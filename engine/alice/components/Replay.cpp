...
void Replay::enqueueNextMessage(const std::string& tag, int64_t prev_timestamp) {
  ...
  job_descriptor.action = [this, tag, message_uuid, target_log_time] {
    ...
    message_ledger_->provide({this, tag}, readMessage(message_uuid));
...
ConstMessageBasePtr Replay::readMessage(const Uuid& message_uuid) const {
  serialization::Header header;
  std::vector<byte> proto_buffer;
  std::vector<SharedAlignedBuffer> buffers;
  cask_->keyValueRead(message_uuid, [&](const byte* begin, const byte* end) {
    ...
    for (const size_t length : header.buffers) {
      CpuAlignedBuffer output(1, length);
  ...
  if (use_recorded_message_time_) {
    ...
    message->acqtime = *header.acqtime - replay_time_offset_ + scheduler_reference_time_;
  }
  return message;
}
