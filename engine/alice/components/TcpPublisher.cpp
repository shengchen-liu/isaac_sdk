...
void TcpPublisher::start() {
  ...
  impl_->acceptor = std::make_unique<asio::ip::tcp::acceptor>(backend_->io_service());
  ...
  if (ec) {
    reportFailure("Failed to bind TCP endpoint at port %d. Is the port already in use? "
                  "Will not publish any data.", port);
    return;
  }
...
void TcpPublisher::acceptConnection() {
  ...
  impl_->pending_socket = std::make_unique<asio::ip::tcp::socket>(backend_->io_service());
  ASSERT(impl_->pending_socket, "Could not create socket");
  impl_->acceptor->async_accept(*impl_->pending_socket,
...
void TcpPublisher::send(ConstMessageBasePtr message, const std::string& channel) {
  ...
  // stream the message
  impl_->stream_message_writer.write(message, time_synchronizer_, channel,
