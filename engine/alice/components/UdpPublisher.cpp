...
void UdpPublisher::start() {
  ...
  asio::ip::udp::resolver resolver(backend_->io_service());
  asio::ip::udp::resolver::query query(
      asio::ip::udp::v4(), host, std::to_string(port));
  impl_->remote_endpoint = *resolver.resolve(query);
  impl_->socket = std::make_unique<asio::ip::udp::socket>(backend_->io_service());
...
void UdpPublisher::async_send(ConstMessageBasePtr message, const std::string& channel) {
  ...
  for (auto& package : buffer->packages()) {
    impl_->socket->async_send_to(package, impl_->remote_endpoint,
        [this, buffer](std::error_code ec, size_t num_sent) {
          if (ec) {
