...
class PyCodeletImageReader(Codelet):
    ...
    def tick(self):
        ...
        assert img_msg.image.rows == 2048, "Incorrect image rows {}".format(img_msg.image.rows)
        assert img_msg.image.cols == 2048, "Incorrect image cols {}".format(img_msg.image.cols)
        buffer_data = self.rx.get_buffer_content(img_msg.image.dataBufferIndex)
        assert len(buffer_data) == 2048 * 2048 * 3, "Incorrect image bytes length {}".format(
            len(buffer_data))
