...
class ImageBase {
 public:
  ...
  : data_(std::move(buffer)), rows_(rows), cols_(cols) {
    ASSERT(rows <= buffer.rows(), "Buffer can not hold enough rows: %zd vs %zd",
           buffer.rows(), rows);
    ASSERT(buffer.stride() >= getUsedBytesPerRow(), "Buffer stride too small: %zd vs %zd",
           buffer.stride(), getUsedBytesPerRow());
  }
  ...
  std::enable_if_t<X, image_view_t> view() {
    return image_view_t({this->data().pointer().get(), this->rows(), this->getStride()},
                        this->rows(), this->cols());
  }
  image_const_view_t const_view() const {
    return image_const_view_t({this->data().pointer().get(), this->rows(), this->getStride()},
                              this->rows(), this->cols());
  }
  ...
  operator image_const_view_t() const {
    return const_view();
  }
  ...
  std::enable_if_t<X, void> resize(size_t desired_rows, size_t desired_cols) {
    ...
    if (rows_ != desired_rows || cols_ != desired_cols) {
      rows_ = desired_rows;
      cols_ = desired_cols;
      data_ = buffer_t(rows_, cols_ * N * sizeof(K));
    }
  }
  template <bool X = kIsOwning>
  ...
  size_t getByteSize() const { return data_.rows() * data_.stride(); }
  ...
  size_t getStride() const { return data_.stride(); }
  ...
  bool hasTrivialStride() const {
    return getStride() == getUsedBytesPerRow();
  }
...
template <class K, int N>
using ImageConstView = ImageBase<K, N, CpuAlignedBufferConstView>;
...
ImageView<K, N> CreateImageView(K* data, size_t rows, size_t cols, size_t stride) {
  return ImageView<K, N>(
      CpuAlignedBufferView(reinterpret_cast<byte*>(data), rows, stride),
      rows, cols);
}

// Helper function to create an image view from a pointer using dense storage
template <class K, int N>
ImageView<K, N> CreateImageView(K* data, size_t rows, size_t cols) {
  return CreateImageView<K, N>(data, rows, cols, cols * N * sizeof(K));
}

// Helper function to create an image const view from a pointer using pitched storage
template <class K, int N>
ImageConstView<K, N> CreateImageView(const K* data, size_t rows, size_t cols, size_t stride) {
  return ImageConstView<K, N>(
      CpuAlignedBufferConstView(reinterpret_cast<const byte*>(data), rows, stride),
      rows, cols);
}
...
template <class K, int N>
using CudaImage = ImageBase<K, N, CudaAlignedBuffer>;
...
template <class K, int N>
using CudaImageView = ImageBase<K, N, CudaAlignedBufferView>;
...
template <class K, int N>
using CudaImageConstView = ImageBase<K, N, CudaAlignedBufferConstView>;
