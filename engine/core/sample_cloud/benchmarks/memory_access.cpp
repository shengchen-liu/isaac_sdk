...
// Helper function to generate planar data
template <size_t Channels>
SampleCloudPlanar<float, Channels> RandomPlanarPointData(size_t num_points) {
  SampleCloudPlanar<float, Channels> out(num_points);
  uint32_t seed;
  for (size_t index = 0; index < num_points; index++) {
    for (size_t i = 0; i < Channels; i++) {
      out[index][i] = rand_r(&seed);
...
// Test reading planar memory and using the full data sample
template <size_t Channels>
void PlanarMemoryFullAccess(benchmark::State& state) {
  size_t num_points = state.range(0);
  SampleCloudPlanar<float, Channels> input = RandomPlanarPointData<Channels>(num_points);
...
// Test single channel accesss for planar memory
template <size_t Channels>
void PlanarMemorySingleChannelAccess(benchmark::State& state) {
  size_t num_points = 0;
  SampleCloudPlanar<float, Channels> input;
  float out = 0.0f;
  num_points = state.range(0);
  input = RandomPlanarPointData<Channels>(num_points);
  for (auto _ : state) {
    for (size_t i = 3; i < num_points - 3; ++i) {
...
// Test reading planar memory and using the full data sample
template <size_t Channels>
void PlanarMemoryFullRandomAccess(benchmark::State& state) {
  size_t num_points = state.range(0);
...
// Test single channel accesss for planar memory
template <size_t Channels>
void PlanarMemorySingleChannelRandomAccess(benchmark::State& state) {
  size_t num_points = state.range(0);
  std::random_device rd;
  std::mt19937 eng(rd());
  std::uniform_int_distribution<> distr(3, num_points - 3);
...
// Test Cuda full channel access for planar memory.
template <size_t Channels>
void CudaPlanarMemoryFullAccess(benchmark::State& state) {
  size_t num_points = state.range(0);
  SampleCloudPlanar<float, Channels> input;
  CudaSampleCloudPlanar<float, Channels> cuda_input(num_points);
  input = RandomPlanarPointData<Channels>(num_points);
  Copy(input, cuda_input);
  for (auto _ : state) {
    float ms =
        WindowedSumPlanar(reinterpret_cast<float*>(cuda_input.data().pointer().get()), Channels,
                          num_points, cuda_input.channel_data_pitch() / sizeof(float));
    state.SetIterationTime(ms / 1000.);
  }
}
...
BENCHMARK_TEMPLATE(CudaInterleavedMemoryFullAccess, 4)
    ...
    ->UseManualTime();
BENCHMARK_TEMPLATE(CudaPlanarMemoryFullAccess, 4)
    ->Range(4096, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseManualTime();
BENCHMARK_TEMPLATE(InterleavedMemoryFullAccess, 4)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemoryFullAccess, 4)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(InterleavedMemorySingleChannelAccess, 4)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemorySingleChannelAccess, 4)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(InterleavedMemoryFullRandomAccess, 4)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemoryFullRandomAccess, 4)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(InterleavedMemorySingleChannelRandomAccess, 4)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemorySingleChannelRandomAccess, 4)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
...
BENCHMARK_TEMPLATE(CudaInterleavedMemoryFullAccess, 8)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseManualTime();
BENCHMARK_TEMPLATE(CudaPlanarMemoryFullAccess, 8)
    ->Range(4096, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseManualTime();
BENCHMARK_TEMPLATE(InterleavedMemoryFullAccess, 8)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemoryFullAccess, 8)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(InterleavedMemorySingleChannelAccess, 8)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemorySingleChannelAccess, 8)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemoryFullRandomAccess, 8)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(InterleavedMemorySingleChannelRandomAccess, 8)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemorySingleChannelRandomAccess, 8)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
...
BENCHMARK_TEMPLATE(CudaInterleavedMemoryFullAccess, 16)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseManualTime();
BENCHMARK_TEMPLATE(CudaPlanarMemoryFullAccess, 16)
    ->Range(4096, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseManualTime();
BENCHMARK_TEMPLATE(InterleavedMemoryFullAccess, 16)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemoryFullAccess, 16)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(InterleavedMemorySingleChannelAccess, 16)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemorySingleChannelAccess, 16)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemoryFullRandomAccess, 16)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(InterleavedMemorySingleChannelRandomAccess, 16)
    ...
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
BENCHMARK_TEMPLATE(PlanarMemorySingleChannelRandomAccess, 16)
    ->Range(16, 16777216)
    ->Unit(benchmark::kNanosecond)
    ->UseRealTime();
