 ...
 public:
  ...
  static SO3 FromNormalizedQuaternion(const Quaternion<K>& quaternion) {
    ASSERT(IsAlmostEqualRelative(quaternion.squaredNorm(), K(1)),
           "Given cos/sin vector is not normalized.");
  ...
  Vector3<K> eulerAnglesRPY() const { return quaternion_.toRotationMatrix().eulerAngles(0, 1, 2); }
  ...
  // Converts euler angle for rotation
  static SO3 FromEulerAnglesRPY(K roll_angle, K pitch_angle, K yaw_angle) {
    SO3 roll = SO3::FromAngleAxis(roll_angle, {1.0, 0.0, 0.0});
    SO3 pitch = SO3::FromAngleAxis(pitch_angle, {0.0, 1.0, 0.0});
    SO3 yaw = SO3::FromAngleAxis(yaw_angle, {0.0, 0.0, 1.0});
