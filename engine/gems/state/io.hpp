...
bool FromProto(StateProto::Reader proto, size_t batch, size_t timeslice,
               const std::vector<isaac::SharedAlignedBuffer>& buffers, State<K, N>& state) {
...
template <typename K, size_t N>
bool FromProto(StateProto::Reader proto, const std::vector<isaac::SharedAlignedBuffer>& buffers,
               State<K, N>& state) {
