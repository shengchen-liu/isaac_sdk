 ...
 public:
  ...
  void open(const std::string& filename, bool is_writing) {
    ...
    checkError(mdb_env_open(lmdb_env_, filename.c_str(),
                            MDB_NOSUBDIR | (is_writing ? 0 : 0*MDB_RDONLY), 0664));
  }
...
void Cask::keyValueWrite(const Uuid& key, const ByteBufferConstView& blob) {
...
void Cask::keyValueWrite(const Uuid& key, const std::vector<ByteBufferConstView>& blobs) {
  ASSERT(is_writing_, "Cask is read only");
...
void Cask::seriesAppend(const Uuid& uuid, const ByteBufferConstView& blob) {
  ASSERT(is_writing_, "Cask is read only");
