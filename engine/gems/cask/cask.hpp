...
class Cask {
 public:
  ...
  void keyValueWrite(const Uuid& key, const ByteBufferConstView& blob);
  void keyValueWrite(const Uuid& key, const std::vector<byte>& blob);
  // Writes a list of blobs sequentially to key-value storage
  void keyValueWrite(const Uuid& key, const std::vector<ByteBufferConstView>& blobs);
  ...
  void seriesAppend(const Uuid& uuid, const ByteBufferConstView& blob);
