...
TEST(tensor_encoder, DenormalizeImage) {
  ...
  Copy(ImageConstView3f(
           CpuAlignedBufferConstView(tensor.data().pointer().get(), src.rows(), src.cols() * 12),
           src.rows(), src.cols()),
       src);
  DenormalizeImage(src.view(), ImageNormalizationMode::kPositiveNegativeUnit, dst.view());
  Copy(dst, cpu_img);
...
TEST(CudaImage, ResizeImageToRoi) {
  ...
  Copy(cpu_img, img1);
  ResizeImageToRoi(img1.const_view(), dst_roi, img2.view());
  Copy(img2, cpu_img);
...
TEST(CudaImage, CropImageToRoi) {
  ...
  Copy(cpu_img, img1);
  CropImageToRoi(img1.const_view(), src_roi, dst_roi, img2.view());
  Copy(img2, cpu_img);
  for (size_t i = 0; i < cpu_img.rows(); ++i) {
    for (size_t j = 0; j < cpu_img.cols(); ++j) {
...
TEST(CudaImage, ResizeWithAspectRatio) {
  ...
  Copy(cpu_img1, img1);
  ResizeWithAspectRatio(img1.const_view(), img2.view());
  Copy(img2, cpu_img2);
...
TEST(tensor_encoder, NormalizeImage) {
  ...
  auto tensor_view = ImageView3f(
      CpuAlignedBufferView(tensor.data().pointer().get(), dst.rows(), dst.cols() * 3 * sizeof(float)),
      dst.rows(), dst.cols());
