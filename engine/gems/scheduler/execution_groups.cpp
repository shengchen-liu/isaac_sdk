...
void ExecutionGroup::generateJobReport() {
  ...
  if (!items.empty()) {
    LOG_INFO("|================================================================================================|");  // NOLINT
    LOG_INFO("|                                 Job Statistics Report (blocking)                               |");  // NOLINT
    LOG_INFO("|================================================================================================|");  // NOLINT
    LOG_INFO("| %-50.50s | %-17s | %6s | %12s |", "Name", "Job Mode", "#Run", "Avg Time");
    LOG_INFO("|------------------------------------------------------------------------------------------------|");  // NOLINT
    for (const auto& stat : job_statistics_pool_) {
      const char* mode_str = "";
      const auto mode = stat.second->descriptor.execution_mode;
      if (mode == ExecutionMode::kBlocking) {
        mode_str = kBlockingLabel;
      } else if (mode == ExecutionMode::kBlockingOneShot) {
        mode_str = kBlockingOneShotLabel;
      } else {
        continue;
      }
      LOG_INFO("| %-50.50s | %-17s | %6d | %9.2f ms |",
               TakeLast(stat.second->descriptor.name, 50).c_str(), mode_str,
               stat.second->num_executed, stat.second->getAverageExecutionTime() * 1000.0);
    }
    LOG_INFO("|================================================================================================|");  // NOLINT
  }
