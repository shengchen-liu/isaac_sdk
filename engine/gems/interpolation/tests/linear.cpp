...
TEST(BilinearApproximatedFunction, precompute) {
  ...
  BilinearApproximatedFunction<double, true, Image1d> interpolation(
      std::move(img), img.rows(), img.cols());
  std::mt19937 gen;
  std::uniform_real_distribution<double> distribution(1.0, static_cast<double>(img.rows() - 1));
...
TEST(BilinearApproximatedFunction, no_precompute) {
  ...
  BilinearApproximatedFunction<double, false, Image1d> interpolation(
      std::move(img), img.rows(), img.cols());
  std::mt19937 gen;
  std::uniform_real_distribution<double> distribution(1.0, static_cast<double>(img.rows() - 1));
  for (int i = 0; i < 1000; i++) {
