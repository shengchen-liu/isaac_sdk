...
struct JsonSerializer<Vector<K, N>> {
  ...
  static void Set(Json& json, const Vector<K, N>& value) {
    std::array<K, N> data;
...
// Parameter support for SopAsset
// Input serialization formats
//   In order of priority TryGet() will look in "json" for:
//     (1) A string representing the name of the asset.
//         Ex. Json json = "my_favorite_robot";
//     (2) A Json object named "name" containing a string representng the name of the asset
//         Ex. Json json;
//             json["name"] = "my_favorite_robot";
//   If neither format is available, std::nullopt will be returned.
// Output serialization format:
//   A string representing the name of the asset.
template <>
struct JsonSerializer<sight::SopAsset> {
  static std::optional<sight::SopAsset> TryGet(const Json& json) {
    // Try to get SopAsset from std::string name
    auto maybe = JsonSerializer<std::string>::TryGet(json);
    if (maybe) {
      return sight::SopAsset::FromName(*maybe);
    }

    // Try to get SopAsset from named attributes for name
    auto maybe_name = TryGetFromMap<std::string>(json, "name");
    if (maybe_name) {
      return sight::SopAsset::FromName(*maybe_name);
    }

    return std::nullopt;
  }
  static void Set(Json& json, const sight::SopAsset& value) {
    ::isaac::serialization::Set(json, value.name());
  }
};
...
template <typename T>
std::optional<T> TryGet(const Json& json) {
  return details::JsonSerializer<T>::TryGet(json);
}

template <typename T>
void Set(Json& json, T value) {
  details::JsonSerializer<T>::Set(json, std::move(value));
}
