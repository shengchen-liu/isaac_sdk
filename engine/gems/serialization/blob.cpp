...
size_t AccumulateLength(const std::vector<ByteBufferConstView>& blobs) {
  size_t length = 0;
...
byte* CopyAll(const std::vector<ByteBufferConstView>& blobs, byte* dst, byte* dst_end) {
  for (const auto& blob : blobs) {
...
void BlobsToLengths32u(const std::vector<ByteBufferConstView>& blobs,
...
void CapnpArraysToBlobs(const kj::ArrayPtr<const kj::ArrayPtr<const ::capnp::word>> segments,
  ...
  for (size_t i = 0; i < blobs.size(); i++) {
    ...
    blobs[i] = ByteBufferConstView{segment.begin(), segment.size()};
  }
}
