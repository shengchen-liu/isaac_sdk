...
uint8_t* WriteToBuffer(uint8_t* buffer, uint8_t* end, const C& source) {
...
template <typename I, typename C>
const uint8_t* ReadFromBuffer(const uint8_t* buffer, const uint8_t* end, C& target) {
  static_assert(std::is_same<I, typename C::value_type>::value, "value types mismatch");
  CountT count_raw;
...
uint8_t* SerializeImpl(const Header& header, bool with_tip, uint32_t* ptr_tip, F resize) {
  ...
  if (!header.tag.empty()) {
    ptr = WriteToBuffer<char>(ptr, ptr_end, header.tag);
  ...
  if (!header.minipayload.empty()) {
    ptr = WriteToBuffer<uint8_t>(ptr, ptr_end, header.minipayload);
    ASSERT(ptr, "logic error: could not write minipayload");
  }
  if (!header.segments.empty()) {
    ptr = WriteToBuffer<uint32_t>(ptr, ptr_end, header.segments);
    ASSERT(ptr, "logic error: could not write segments");
  }
  if (!header.buffers.empty()) {
    ptr = WriteToBuffer<uint32_t>(ptr, ptr_end, header.buffers);
    ASSERT(ptr, "logic error: could not write buffers");
  }
  return ptr;
}

const uint8_t* DeserializeImpl(const uint8_t* ptr, const uint8_t* ptr_end,
                               uint32_t* ptr_tip, Header& header) {
  // read magic byte
  uint32_t magic;
  if (ptr_tip) {
    magic = *ptr_tip;
  ...
  if (magic & TIP_3_TAG) {
    ptr = ReadFromBuffer<char>(ptr, ptr_end, header.tag);
    if (!ptr) return nullptr;
  } else {
    header.tag.clear();
  }
  ...
  if (magic & TIP_6_MINIPAYLOAD) {
    ptr = ReadFromBuffer<uint8_t>(ptr, ptr_end, header.minipayload);
    if (!ptr) return nullptr;
  } else {
    header.minipayload.clear();
  }
  if (magic & TIP_7_SEGMENTS) {
    ptr = ReadFromBuffer<uint32_t>(ptr, ptr_end, header.segments);
    if (!ptr) return nullptr;
  } else {
    header.segments.clear();
  }
  if (magic & TIP_8_BUFFERS) {
    ptr = ReadFromBuffer<uint32_t>(ptr, ptr_end, header.buffers);
    if (!ptr) return nullptr;
  } else {
...
bool Size(const Header& header, bool with_tip, size_t* size, uint32_t* tip) {
  ...
  if (!header.tag.empty()) {
    magic |= TIP_3_TAG;
    ptr = WriteToBuffer<char>(ptr, nullptr, header.tag);
    if (!ptr) return false;
  }
  if (header.acqtime) {
    magic |= TIP_4_ACQTIME;
  ...
  if (!header.minipayload.empty()) {
    ...
    ptr = WriteToBuffer<uint8_t>(ptr, nullptr, header.minipayload);
    if (!ptr) return false;
  }
  if (!header.segments.empty()) {
    magic |= TIP_7_SEGMENTS;
    ptr = WriteToBuffer<uint32_t>(ptr, nullptr, header.segments);
    if (!ptr) return false;
  }
  if (!header.buffers.empty()) {
    magic |= TIP_8_BUFFERS;
    ptr = WriteToBuffer<uint32_t>(ptr, nullptr, header.buffers);
    if (!ptr) return false;
  }
  if (with_tip) {
...
bool Serialize(const Header& header, std::vector<uint8_t>& buffer) {
  return SerializeImpl(header, true, nullptr,
      [&](size_t n) { buffer.resize(n); return buffer.data(); }) != nullptr;
}
...
bool SerializeWithoutTip(const Header& header, uint32_t magic, std::vector<uint8_t>& buffer) {
  ...
  const bool read_ok = SerializeImpl(header, false, &magic_actual,
      [&](size_t n) { buffer.resize(n); return buffer.data(); }) != nullptr;
...
uint8_t* SerializeWithoutTip(const Header& header, uint32_t magic, uint8_t* begin, uint8_t* end) {
  uint32_t magic_actual;
  uint8_t* ptr = SerializeImpl(header, false, &magic_actual,
      [=](size_t n) -> uint8_t* {
        if (begin + n > end) {
          return nullptr;
        }
